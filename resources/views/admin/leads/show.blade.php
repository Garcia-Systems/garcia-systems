<x-admin.layout :heading="'Lead / '.($lead->name ?: $lead->email)">
    ...
    <div class="mt-8 grid gap-6 lg:grid-cols-2">
        ...
        <section class="rounded-xl border border-slate-700 bg-slate-900 p-5"><h2 class="font-semibold text-white">Related assessments</h2><div class="mt-4 space-y-4">@forelse($lead->assessments as $assessment)<article class="text-sm text-slate-300"><p class="font-medium text-white">Score: {{ $assessment->score }} · {{ $assessment->result_tier }}</p><p>{{ $assessment->summary }}</p>@if($assessment->category_scores)<div class="mt-3 grid gap-2">@foreach($assessment->category_scores as $category)<p class="rounded border border-slate-700 bg-slate-950/50 px-3 py-2"><span class="text-slate-400">{{ $category['label'] ?? 'Readiness area' }}:</span> {{ $category['score'] ?? '—' }} / {{ $category['max_score'] ?? 5 }}</p>@endforeach</div>@endif<p class="mt-1 text-slate-400">{{ $assessment->created_at?->format('M j, Y') }}</p></article>@empty<p class="text-sm text-slate-400">No assessments.</p>@endforelse</div></section>
    </div>
</x-admin.layout>
